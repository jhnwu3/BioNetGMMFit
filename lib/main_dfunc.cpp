...
int main(int argc, char** argv){
    ...
    if(outPathExists(argc, argv)){
        outPath = getOutputPath(argc,argv);       
    }
    ...
    parameters.useSBML = modelPathExists(argc,argv);
    ...
    if(parameters.useLinear > 0){
        ...
        for(int y = 0; y < Yt3Mats.size(); ++y){ 
            if(graphingEnabled(argc,argv)){
                matrixToCsvWithLabels(Yt3Mats[y], speciesNames, outPath + file_without_extension + "Yt" + to_string_with_precision(times(y + 1), 2));
            }
    ...
    if(graphingEnabled(argc, argv)){
        for(int t = 1; t < times.size(); t++){
            matrixToCsvWithLabels(Xt3Mats[t-1], speciesNames, outPath + file_without_extension + "Xt" +to_string_with_precision(times(t),2));
        }
        parameterNames.push_back("cost");
        matrixToCsvWithLabels(GBVECS, parameterNames, outPath + file_without_extension + "_estimates");
    }
